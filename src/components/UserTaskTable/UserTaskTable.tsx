...
//   const autoSaveToLocalStorage = (nextData: TaskRow[]) => {
//   try {
//     const cloned = JSON.parse(JSON.stringify(nextData));
//     localStorage.setItem(
//       `user-task-data-${adminTemplate.id}`,
//       JSON.stringify(cloned)
//     );
//     setLastSavedData(cloned);
//   } catch (err) {
//     console.error("Auto-save failed", err);
//   }
// };
// const updateData = (updater: (prev: TaskRow[]) => TaskRow[]) => {
//   setData(prev => {
//     const next = updater(prev);
//     autoSaveToLocalStorage(next);
//     return next;
//   });
// };
...
if (step.completed) {
  ...
  return (
    <div className={`flex flex-col items-start w-full   ${!callingFromAdmin && "pt-4 "} ${callingFromAdmin && "border border-gray-700 p-2 rounded-md"}`}>
      ...
      <AddTaskLine adminTemplate={adminTemplate} data={data} setData={setData} callingFromAdmin={callingFromAdmin} updateData={updateData}/>
      ...
      <Card className={`w-full ${!callingFromAdmin && "mt-2"} `}>
        <CardHeader className="w-full flex flex-row items-center justify-between">
          <CardTitle>{adminTemplate.name || "Task Progress Table"}</CardTitle>
          <TaskSorter data={data} setData={setData} />
        </CardHeader>
